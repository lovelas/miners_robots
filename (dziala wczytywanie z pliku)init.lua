...
--    local file = io.open("/home/daniel/comand_in.txt", "r")
--          io.input(file)
--          wpis = io.read()
    --current page
...
function simple_robots.meta_to_program(meta)
    local ser={}
  local file = io.open("/home/daniel/comand.txt", "a")
    for x=1,CODELINES do
        ser[x]={}
        ser[x].op=meta:get_string("program_"..x.."_op")
        ser[x].msg=meta:get_string("program_"..x.."_msg")
    
  file:write(meta:get_string("program_"..x.."_op"))
  file:write(meta:get_string("program_"..x.."_msg"))
